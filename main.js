...
console.log('foo')
let $ = a => document.getElementById(a), lb = $('lb'), search = $('search__inp'), stateSelection = '', breadCrumb = $('search__breadcrumb'), searchTab = $('search__tab'), stubPage = 0, cityData = [];
...
class State {
    constructor(stateName,stateAbb,stateCities) {
        this.buttonEl = document.createElement('li');
        this.buttonEl.innerHTML = stateName;
    ...
    onButtonClick() {
        ...
        stateSelection = this.stateAbb;
        search.value = '';
        search.focus();
        breadCrumb.hidden = false;
        breadCrumb.innerHTML = `<i class="fa-solid fa-arrow-left brd_trg"></i><span class="brd_trg copy b text-sm">${this.stateName}</span>`;
        lb.scrollTop = 0;
    }
}
...
class City {
    constructor(cityName) {
        ...
        this.buttonEl = document.createElement('li');
        this.buttonEl.innerHTML = cityName[0];
    ...
    onButtonClick() {
        ...
        let cityName = this.cityName, stateName = stateSelection;
        cityData = cityName;
        $('resultsLoc').innerHTML = cityName[0] + ', ' + stateName;
        getCities(cityName);
    }
}
...
let getCities = (citySelection) =>{
    ...
    // console.log("results3.php?st="+stateSelection+"&pg="+stubPage+"&ct="+citySelection[0]+"&zp="+citySelection[1])
    xmlhttp.open("GET", "results3.php?st="+stateSelection+"&pg="+stubPage+"&ct="+citySelection[0]+"&zp="+citySelection[1], true);
    xmlhttp.send();
    console.log(Math.abs(91411 - citySelection[1]),stubPage)
    if(Math.abs(90000 - citySelection[1]) < 3600 && stubPage == 0){$('ban__buck').hidden = false;} else {$('ban__buck').hidden = true;}
}
...
cities.forEach(cit => new State(cit[0],cit[1],cit[2]) );
